...
export function PriceTab({ register, control, errors, watch, purpose }: PriceTabProps) {
    // Rental period only applies to rent properties
    const isRentProperty = purpose?.toLowerCase() === 'rent';

    return (
        <div className="space-y-6">
            {/* Row 1: Price and Rental Period (Rental Period only for Rent properties) */}
            <div className={cn("grid gap-8", isRentProperty ? "grid-cols-2" : "grid-cols-1")}>
                ...
                {/* Rental Period - Only shown for Rent properties */}
                {isRentProperty && (
                    <div className="space-y-2.5">
